...
GDB := gdb
VALGRIND := valgrind
VALGRINDFLAGS := --leak-check=full --track-origins=yes --read-var-info=yes --time-stamp=yes
VALGRIND_LOGDIR := valgrind-reports
...
acceptance-test: $(EXECUTABLE) | $(VALGRIND_LOGDIR)
	VALGRIND=$(VALGRIND) VALGRINDFLAGS="$(VALGRINDFLAGS)" VALGRIND_LOGDIR=$(VALGRIND_LOGDIR) EXECUTABLE=./$(EXECUTABLE) $(PYTEST) $(SRCACCEPTANCETESTDIR)

$(EXECUTABLE): $(OBJ_EXECUTABLE) $(OBJ_MAIN)
	$(CC) $(CPPFLAGS) $(CFLAGS) -o $@ $< $(OBJ_MAIN) $(LDFLAGS)

$(OBJMAINDIR) $(OBJUNITTESTDIR) $(DEPUNITTESTDIR) $(DEPMAINDIR) $(VALGRIND_LOGDIR):
	mkdir -p $@
